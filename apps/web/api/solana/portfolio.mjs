...
export default async function handler(req, res) {
  ...
  if (!isSolanaAddress(address)) return json(res, 400, { error: "Invalid Solana wallet address", code: "INVALID_ADDRESS" });
  const rpcUrl = process.env.SOLANA_RPC_URL?.trim();
  if (!rpcUrl) return json(res, 503, { error: "Live Solana balances are not configured", code: "RPC_NOT_CONFIGURED" });
  try {
    const parsed = new URL(rpcUrl);
    if (parsed.protocol !== "https:") throw new Error("invalid-config");
  } catch {
    return json(res, 503, { error: "Live Solana balances are not configured", code: "RPC_INVALID_CONFIG" });
  }
  try {
    ...
    return json(res, 502, {
      ...
      code: reason === "upstream-http" ? "RPC_HTTP_ERROR" : reason === "TimeoutError" ? "RPC_TIMEOUT" : "RPC_RESPONSE_ERROR",
    });
  }
}
