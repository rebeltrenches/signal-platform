...
import { getWalletHistoryReplayRoute } from '../src/routes/wallets.js';
...
async function run() {
  const unsupported = await getWalletHistoryReplayRoute(request('ETHEREUM', 'WalletA'));
  assert.equal(unsupported.status, 400);
  assert.match(String((unsupported.body as any).message), /SOLANA, BASE, or BNB/);

  const blank = await getWalletHistoryReplayRoute(request('SOLANA', '   '));
  assert.equal(blank.status, 400);
  assert.match(String((blank.body as any).message), /wallet address is required/i);

  for (const depth of ['0', '7', '1.5', 'abc']) {
    const response = await getWalletHistoryReplayRoute(request('SOLANA', 'WalletA', depth));
  ...
  try {
    const unavailable = await getWalletHistoryReplayRoute(request('SOLANA', 'WalletA', '3'));
    assert.equal(unavailable.status, 404);
    assert.equal((unavailable.body as any).error, 'NOT_FOUND');
    assert.match(String((unavailable.body as any).message), /History Replay storage is not available/i);
  } finally {
  ...
  console.log('  ok  - validates replay depth');
